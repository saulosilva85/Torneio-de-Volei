...
st.title("🏐 Torneio de Vôlei - Sorteio + PDF")
...
def gerar_pdf(times, nomes_times, tabela):
    doc = SimpleDocTemplate("torneio_volei.pdf")
    styles = getSampleStyleSheet()
    elementos = []

    elementos.append(Paragraph("🏐 TORNEIO DE VÔLEI", styles["Title"]))
...
if st.button("🎲 Sortear + Gerar PDF"):
    ...
    # Gerar PDF
    gerar_pdf(times, nomes_times, tabela)

    # Botão de download
    with open("torneio_volei.pdf", "rb") as f:
        st.download_button(
            label="📄 Baixar PDF",
            data=f,
            file_name="torneio_volei.pdf",
